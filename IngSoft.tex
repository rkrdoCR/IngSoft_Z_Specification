...
La operación garantiza ser robusta ya que se combina con la especificación \textit{ASIGNA} y cubre las dos posibles salidas.
\begin{enumerate}
...
En el siguiente diagrama se puede apreciar el estado del sistema luego de que esta operación se completo de manera satisfactoria.
...
\subsubsection*{Entrega fallida.}
Por el contrario, si el paquete no pudo entregarse al comprador el mismo debe de mantenerse asignado al mensajero hasta que este sea devuelto a la bodega, por lo que el sistema mantiene el paquete en ruta pero cambia la respuesta obtenida en la operación.
...
El cuadro que se presenta a continuacion resume el funcionamiento del sistema para cuando la entrada es de tipo \textit{Paquete}.
